...
/** playbook 없을 때 법령명만 있을 경우 대표 조문(본문 조회용) */
const DEFAULT_JO_BY_LAW = {
  민법: "제750조",
  도로교통법: "제48조",
  근로기준법: "제2조",
  형법: "제21조",
...
function defaultJoForLaw(lawName) {
  return DEFAULT_JO_BY_LAW[lawName] || null;
}

async function enrichLawHitWithArticle(hit) {
  if (!hit?.mst || (hit.desc && hit.clause && hit.clause !== "관련 조문")) {
    return hit;
  }
  const jo = hit.jo || defaultJoForLaw(hit.name);
  if (!jo) return null;
  const parsed = await getArticleText(hit.mst, jo);
  ...
  return {
    ...
    desc: parsed.snippet,
    link: `https://www.law.go.kr/법령/${encodeURIComponent(hit.name)}/${jo}`
  };
}
...
function useHttpLawApi() {
  return (
    ...
    process.env.LAW_API_MODE === "http" ||
    process.env.USE_LAW_HTTP === "1"
  );
}
