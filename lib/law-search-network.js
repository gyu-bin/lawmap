...
function articleBlob(hit) {
  return `${hit.name} ${hit.clause || ""} ${hit.desc || ""}`;
}

function matchesSearchQuery(blob, query) {
  const q = String(query || "").trim();
  if (!q) return false;
  if (blob.includes(q)) return true;
  const parts = q.split(/\s+/).filter((p) => p.length >= 2);
  if (!parts.length) return false;
  if (parts.length === 1) return blob.includes(parts[0]);
  return parts.filter((p) => blob.includes(p)).length >= 2;
}
...
function formatUnitAsHit(lawName, mst, unit) {
  ...
  return {
    ...
    desc: cleanLawBody(body) || body.slice(0, 220),
...
function scoreArticleHit(hit, searchQueries) {
  let score = 0;
  if (/시행령|시행규칙/.test(hit.name)) score -= 20;
  const blob = articleBlob(hit);
  for (const q of searchQueries) {
    if (matchesSearchQuery(blob, q)) score += 22;
    else if (q.split(/\s+/).some((p) => p.length >= 2 && blob.includes(p))) score += 8;
  }
  return score;
}

function isRelevantArticleHit(hit, searchQueries) {
  return searchQueries.some((q) => matchesSearchQuery(articleBlob(hit), q));
}

async function searchSituationViaNetwork(userText, analysis = {}) {
  const searchQueries =
    analysis.lawSearchQueries?.length > 0
      ? analysis.lawSearchQueries
      : extractSearchQueries(userText);
  const explicitJo = extractJoFromText(userText);
  let hits = [];
  const tried = new Set();

  const runAiSearch = async (q) => {
    if (tried.has(q)) return [];
    tried.add(q);
    const raw = await fetchAiArticleSearch(q);
    return raw.filter((h) => isRelevantArticleHit(h, searchQueries));
  };

  for (const q of searchQueries) {
    try {
      hits = dedupeHits([...hits, ...(await runAiSearch(q))]);
    } catch (err) {
  ...
  if (hits.length < 5) {
    ...
    for (const law of laws) {
      try {
        const row =
          (await resolveArticleForLaw(law, userText, explicitJo)) ||
          (await findArticleByKeywords(law.mst, law.name, searchQueries));
        if (row?.desc && isRelevantArticleHit(row, searchQueries)) hits.push(row);
      } catch (err) {
  ...
  hits = hits
    .filter((h) => isRelevantArticleHit(h, searchQueries))
    .map((h) => ({ ...h, _score: scoreArticleHit(h, searchQueries) }))
    .filter((h) => h._score > 0)
    .sort((a, b) => b._score - a._score);

  return { hits: dedupeHits(hits).slice(0, 5), queries: searchQueries };
}
